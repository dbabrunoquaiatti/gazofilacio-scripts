...
def processar_imagem(caminho_imagem, pessoas):
    ...
    try:
        ...
        # Valida que é um comprovante do Santander
        if extrair_instituicao_do_texto:
            inst_info = extrair_instituicao_do_texto(linhas)
            if inst_info.get("instituicao") and inst_info["instituicao"] != "SANTANDER":
                resultado["erro"] = f"Comprovante não é do Santander, é de {inst_info.get('instituicao')}"
                return resultado
