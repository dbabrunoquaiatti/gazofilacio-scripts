...
# Padrão de data melhorado - detecta: "05 JAN 2026", "5 JAN 2026", etc
...
def converter_data_textual(data_texto):
    ...
    data_texto = data_texto.replace("\u00a0", " ")  # outro tipo de espaço unicode
    data_texto = re.sub(r"\s+", " ", data_texto).strip()
    
    # Padrão mais flexível: DD MES AAAA, pode ter espaços variados
    # Tenta primeiro: \d{1,2} + LETRAS + \d{4}
    match = re.search(r"(\d{1,2})\s+([A-Z\u00C0-\u00FC]+)\s+(\d{4})", data_texto, re.IGNORECASE)
    if match:
        ...
        if mes_texto_norm in MESES_ABREV:
            ...
            ano_2dig = ano[-2:]  # pega os 2 últimos dígitos do ano
            return f"{dia}/{mes}/{ano_2dig}"
    
    # Fallback: tenta sem espaço obrigatório entre dia e mês (OCR pode comer espaços)
    match = re.search(r"(\d{1,2})\s*([A-Z\u00C0-\u00FC]+)\s*(\d{4})", data_texto, re.IGNORECASE)
    if match:
        dia, mes_texto, ano = match.groups()
        mes_texto_norm = normalizar(mes_texto)
...
def validar_data_no_texto(texto):
    ...
    texto_limpo = texto.replace("\u00a0", " ")  # outro tipo de espaço unicode
    texto_limpo = re.sub(r"\s+", " ", texto_limpo)

    # Corrige erros comuns de OCR onde dígitos são confundidos com letras
    # Procura por padrões específicos de erro como "OS", "OI", "IL", "I5", "S5" seguidos de mês
    texto_corrigido = texto_limpo
    
    # Mapa de substituições para erros comuns de OCR: padrão inválido -> número correto
    # Formato: (padrão_errado, dígitos_corretos)
    erros_ocr = [
        (r'\bOS\s+', '03 '),   # OS (letra O + letra S) -> 03 (OCR confunde 3 com O e S)
        (r'\bOI\s+', '01 '),   # OI (letra O + letra I) -> 01
        (r'\bOL\s+', '01 '),   # OL (letra O + letra l) -> 01
        (r'\bIl\s+', '11 '),   # Il (letra I + letra l) -> 11
        (r'\bS\s+', '5 '),     # S único -> 5
        (r'\bZ\s+', '2 '),     # Z -> 2
    ]
    
    for padrao_errado, digitos_corretos in erros_ocr:
        # Verifica se o padrão de erro é seguido por um mês
        if re.search(padrao_errado + r'[A-Z\u00C0-\u00FC]{2,}\s+\d{4}', texto_corrigido, re.IGNORECASE):
            texto_corrigido = re.sub(padrao_errado, digitos_corretos, texto_corrigido, flags=re.IGNORECASE)

    # PRIMEIRO: tenta buscar direto padrão DD MES AAAA (mais comum)
    # Usa padrão mais flexível para capturar variações de espaço do OCR
    m = re.search(r"(\d{1,2})\s+([A-Z\u00C0-\u00FC]+)\s+(\d{4})", texto_corrigido, re.IGNORECASE)
    if m:
        dia_str = m.group(1).zfill(2)
        mes_str = m.group(2)
        ano_str = m.group(3)  # sempre group 3 (único ano no padrão)
        
        mes_norm = normalizar(mes_str)
        if mes_norm in MESES_ABREV:
            mes_num = MESES_ABREV[mes_norm]
            ano_2dig = ano_str[-2:]
            return f"{dia_str}/{mes_num}/{ano_2dig}"
    
    # SEGUNDO: tenta padrão com 4 dígitos de ano (DD/MM/YYYY ou DD-MM-YYYY)
    # Prioriza padrões com ano de 4 dígitos para evitar confusão com anos de 2 dígitos
    padrao_data_4dig = r"\b\d{2}/\d{2}/\d{4}\b|\b\d{2}-\d{2}-\d{4}\b|\b\d{1,2}\s+(?:JANEIRO|FEVEREIRO|MARCO|MARÇO|ABRIL|MAIO|JUNHO|JULHO|AGOSTO|SETEMBRO|OUTUBRO|NOVEMBRO|DEZEMBRO|JAN|FEV|MAR|ABR|MAI|JUN|JUL|AGO|SET|OUT|NOV|DEZ)\s+\d{4}\b"
    m = re.search(padrao_data_4dig, texto_limpo, re.IGNORECASE)
    if m:
        found = m.group(0)
        ...
        # normaliza formatação DD/MM/YYYY -> DD/MM/YY
        ...
        # normaliza formatação DD-MM-YYYY -> DD/MM/YY
        if re.match(r"\d{2}-\d{2}-\d{4}", found):
            partes = found.replace('-', '/').split('/')
            return f"{partes[0]}/{partes[1]}/{partes[2][-2:]}"
    
    # TERCEIRO: tenta padrão com 2 dígitos de ano (DD/MM/YY)
    # SÓ DEPOIS de procurar por 4 dígitos para evitar falsos positivos
    m = re.search(r"\b\d{2}/\d{2}/\d{2}\b", texto_limpo, re.IGNORECASE)
    if m:
        found = m.group(0)
        partes = found.split('/')
        dia, mes, ano_2dig = partes[0], partes[1], partes[2]
        
        # Valida se o ano faz sentido
        # Se for muito futuro (> 30), procura por "20XX" no texto como referência
        ano_num = int(ano_2dig)
        if ano_num > 30:
            # Procura por "20XX" no texto (de preferência próximo à data)
            m_ano = re.search(r'\b20(\d{2})\b', texto_limpo)
            if m_ano:
                ano_2dig = m_ano.group(1)
    ...
    try:
        ...
        if os.environ.get("DEBUG_OCR"):
            ...
            with open(debug_path, "w", encoding="utf-8") as dbg:
                ...
                dbg.write("\n\n--DATAS ENCONTRADAS PELO PADRAO--\n")
                datas_encontradas = re.findall(PADRAO_DATA, texto, re.IGNORECASE)
                dbg.write(str(datas_encontradas))
                dbg.write("\n\n--PADRAO FLEXIVEL DE DATA (DD MES AAAA)--\n")
                datas_flexivel = re.findall(r"(\d{1,2})\s+([A-Z\u00C0-\u00FC]+)\s+(\d{4})", texto, re.IGNORECASE)
                dbg.write(str(datas_flexivel))
