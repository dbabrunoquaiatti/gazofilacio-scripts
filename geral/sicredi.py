...
def extrair_nome_do_texto(linhas):
    ...
    # 0) Procura por "Nome do pagador:" específico para SICREDI
    for i, ln in enumerate(linhas):
        if "NOME DO PAGADOR" in normalizar(ln):
            # Tenta extrair da mesma linha (formato "Nome do pagador: EUZA CARDOSO...")
            m = re.search(r"(?:Nome do pagador)\s*:?\s*(.+?)(?:\n|$)", ln, re.IGNORECASE)
            if m:
                nome_raw = m.group(1).strip()
                if nome_raw and re.search(r"[A-Za-zÀ-ú]", nome_raw):
    ...
    # 1) Procura por "Comprovante de Pix" -> "De" -> próxima linha é o nome (específico para PicPay)
    idx_comprovante = -1
    for i, ln in enumerate(linhas):
    ...
    # 2) Procura "Dados do pagador" -> "De" -> próxima linha é o nome
    idx_pagador = -1
    for i, ln in enumerate(linhas):
...
def processar_imagem(caminho_imagem, pessoas):
    ...
    try:
        ...
        # Valida que é um comprovante do SICREDI
        if extrair_instituicao_do_texto:
            inst_info = extrair_instituicao_do_texto(linhas)
            if inst_info.get("instituicao") and inst_info["instituicao"] != "SICREDI":
                resultado["erro"] = f"Comprovante não é do SICREDI, é de {inst_info.get('instituicao')}"
                return resultado
