...
def extrair_nome_do_texto(linhas):
    ...
    # 1) Busca "De" seguido de nome (sem precisar de "Dados do pagador")
    for i, ln in enumerate(linhas):
        if normalizar(ln) == "DE":
            ...
            if j < len(linhas):
                ...
                # Valida: tem nome válido e não começa com "CPF"
                if len(nome_raw.split()) >= 2 and not re.search(r"^CPF", nome_raw, re.IGNORECASE):
                    nome_norm = normalizar(nome_raw)
                    nome = re.sub(r"[^A-Z\s]", "", nome_norm)
                    if nome.strip():
                        return nome

    # 2) Origem tem prioridade: tenta mesma linha primeiro
    ...
    # 3) Se 'Origem' aparece em linha isolada, pega próxima linha como nome
    for i, ln in enumerate(linhas):
