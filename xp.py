...
def extrair_nome_do_texto(linhas):
    ...
    # 1) XP: "Enviado de" tem prioridade absoluta
    m = re.search(r"Enviado\s*de\s*(.+?)(?:\n|$)", texto, re.IGNORECASE)
    if m:
    ...
    # 2) Se 'Enviado de' aparece em linha isolada, pega próxima linha não vazia como nome
    for i, ln in enumerate(linhas):
        if "ENVIADO" in normalizar(ln) and "DE" in normalizar(ln):
            # tenta conteúdo na mesma linha após 'Enviado de'
            m2 = re.search(r"Enviado\s+de\s*(.+)$", ln, re.IGNORECASE)
            ...
            if j < len(linhas):
                ...
                candidato_limpo = re.sub(r"[^A-Z\s]", "", candidato_norm)
                if not re.search(r"CPF|CNPJ|Instituição|Agência", candidato, re.IGNORECASE):
                    return candidato_limpo if len(candidato_limpo.split()) >= 2 else None

    # 3) Fallback: procura "Origem" (padrão de outros bancos)
