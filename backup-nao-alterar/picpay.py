...
PADRAO_DATA  = r"\b\d{1,2}\s*(?:JANEIRO|FEVEREIRO|MARCO|MARÇO|ABRIL|MAIO|JUNHO|JULHO|AGOSTO|SETEMBRO|OUTUBRO|NOVEMBRO|DEZEMBRO|JAN|FEV|MAR|ABR|MAI|JUN|JUL|AGO|SET|OUT|NOV|DEZ)\s*\d{4}\b|\b\d{2}/\d{2}/\d{4}\b|\b\d{2}/\d{2}/\d{2}\b|\b\d{2}-\d{2}-\d{4}\b|\b\d{1,2}/[A-Za-zÀ-ú]{2,3}/\d{4}\b"
...
def extrair_nome_do_texto(linhas):
    ...
    # 0) Procura por "Comprovante de Pix" -> "De" -> próxima linha é o nome (específico para PicPay)
    idx_comprovante = -1
    for i, ln in enumerate(linhas):
        if "COMPROVANTE DE PIX" in normalizar(ln):
            idx_comprovante = i
            break
    
    if idx_comprovante != -1:
        # Procura "De" após "Comprovante de Pix"
        for i in range(idx_comprovante, min(idx_comprovante + 15, len(linhas))):
            if normalizar(linhas[i]) == "DE":
                # Próxima linha não vazia é o nome
                j = i + 1
                while j < len(linhas) and not linhas[j].strip():
                    j += 1
                if j < len(linhas):
                    nome_raw = linhas[j].strip()
                    if len(nome_raw.split()) >= 2 and not "CPF" in normalizar(linhas[j]):
                        nome_norm = normalizar(nome_raw)
                        nome = re.sub(r"[^A-Z\s]", "", nome_norm)
                        if nome.strip():
                            return nome

    # 1) Procura "Dados do pagador" -> "De" -> próxima linha é o nome
    idx_pagador = -1
    for i, ln in enumerate(linhas):
        if "DADOS DO PAGADOR" in normalizar(ln):
            idx_pagador = i
            break
    
    if idx_pagador != -1:
        # Procura "De" após "Dados do pagador"
        for i in range(idx_pagador, min(idx_pagador + 10, len(linhas))):
            if normalizar(linhas[i]) == "DE":
                # Próxima linha não vazia é o nome
                j = i + 1
                while j < len(linhas) and not linhas[j].strip():
    ...
    # 2) Origem tem prioridade absoluta: tenta mesma linha primeiro
    ...
    # 4) Fallback: procura todas as ocorrências de 'Nome' e prefere a última
...
def validar_data_no_texto(texto):
    ...
    if m:
        ...
        # trata formato DD/MÊS/YYYY (ex: 31/mar/2026)
        match_slash_mes = re.match(r"(\d{1,2})/([A-Za-zÀ-ú]{2,3})/(\d{4})", found, re.IGNORECASE)
        if match_slash_mes:
            dia, mes_texto, ano = match_slash_mes.groups()
            mes_texto_norm = normalizar(mes_texto)
            dia = dia.zfill(2)
            if mes_texto_norm in MESES_ABREV:
                mes = MESES_ABREV[mes_texto_norm]
        ...
        if re.match(r"\d{2}/\d{2}/\d{4}", found):
            return found[:6] + found[-2:]  # DD/MM/ (6 chars) + YY (últimos 2)
...
def processar_imagem(caminho_imagem, pessoas):
    ...
    try:
        ...
        # Valida que é um comprovante do PICPAY
        if extrair_instituicao_do_texto:
            inst_info = extrair_instituicao_do_texto(linhas)
            if inst_info.get("instituicao") and inst_info["instituicao"] != "PICPAY":
                resultado["erro"] = f"Comprovante não é do PICPAY, é de {inst_info.get('instituicao')}"
                return resultado
...
def processar_pdf(caminho_pdf, pessoas):
...
def main():
    ...
    if pytesseract is None and pdfplumber is None:
        print("pytesseract e pdfplumber não encontrados.")
        return 1

    # Se houver argumentos de linha de comando, usa-os
    if len(sys.argv) > 1:
        arquivos = sys.argv[1:]
    else:
    ...
    for f in arquivos:
        ...
        if f.lower().endswith(".pdf"):
            if pdfplumber is None:
                print(f"{f}: ERRO - pdfplumber não disponível")
                continue
            caminho = os.path.join(DATA_DIR, f)
            r = processar_pdf(caminho, pessoas)
        else:
            if pytesseract is None:
                print(f"{f}: ERRO - pytesseract não disponível")
                continue
            caminho = os.path.join(DATA_DIR, f)
