...
def extrair_nome_do_texto(linhas):
    ...
    # 1) BB: procura por "Pagador" nos dados pessoais - próxima linha é o nome
    for i, ln in enumerate(linhas):
        if "PAGADOR" in normalizar(ln):
            ...
            if j < len(linhas):
                ...
                if not re.search(r"agencia|conta|instituicao|tipo|chave|crf|eme|crr", candidato, re.IGNORECASE):
    ...
    # 5) coluna: 'Nome' à esquerda e valor à direita (busca por linhas individuais)
    for ln in linhas:
