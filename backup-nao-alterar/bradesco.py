...
def extrair_valor_correto(texto):
    ...
    # 1) Tenta primeiro padrão "Valor:" seguido de número (com ou sem milhar/centavos)
    ...
    # 2) Padrão genérico com R$ (suporta milhar e centavos opcionais)
    pattern = re.compile(r"R\$\s*(\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{2})?)", re.IGNORECASE)
