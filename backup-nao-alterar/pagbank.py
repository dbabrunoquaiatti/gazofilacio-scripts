...
#PASTA_IMAGENS = "C:\\Users\\brunoquaiatti\\Documents\\automacao-dizimo\\data"
...
def extrair_nome_do_texto(linhas):
    ...
    # 1) PagBank: "De" tem prioridade absoluta (seção do pagador)
    m = re.search(r"^De\s*$", texto, re.MULTILINE | re.IGNORECASE)
    if m:
        # Procura próxima linha não vazia após "De"
        linhas_norm = [l.strip() for l in texto.split('\n')]
        for i, ln in enumerate(linhas_norm):
            if re.match(r"^De$", ln, re.IGNORECASE):
                j = i + 1
                while j < len(linhas_norm) and not linhas_norm[j].strip():
                    j += 1
                if j < len(linhas_norm):
                    candidato = linhas_norm[j].strip()
                    if re.search(r"[A-Za-zÀ-ú]", candidato) and not re.search(r"CPF|CNPJ", candidato, re.IGNORECASE):
    ...
    # 2) Fallback: procura "De" seguido de nome na próxima linha (pela lista de linhas original)
    for i, ln in enumerate(linhas):
        if re.match(r"^De$", ln.strip(), re.IGNORECASE):
            if i + 1 < len(linhas):
    ...
    # 3) Fallback: procura "De" seguido de nome (padrão com regex)
    m = re.search(r"(?:^|\n)De\s+([A-Z\u00C0-\u00FC\s]+?)(?:\s+CPF|\s+CNPJ|\s+CpF|\n)", texto, re.IGNORECASE | re.MULTILINE)
    if m:
    ...
    # coluna: 'Nome' à esquerda e valor à direita
    for ln in linhas:
