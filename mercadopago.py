...
#JSON_PESSOAS  = "C:\\Users\\brunoquaiatti\\Documents\\automacao-dizimo\\data\\pessoas.json"


# PADROES
PADRAO_VALOR = r"(?:R\$\s*)?(\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{2})?)\b"
PADRAO_DATA  = r"\b\d{1,2}[/\s]*(?:de)?\s*(?:JANEIRO|FEVEREIRO|MARCO|MARÇO|ABRIL|MAIO|JUNHO|JULHO|AGOSTO|SETEMBRO|OUTUBRO|NOVEMBRO|DEZEMBRO|JAN|FEV|MAR|ABR|MAI|JUN|JUL|AGO|SET|OUT|NOV|DEZ)[/\s]*(?:de)?\s*\d{4}\b|\b\d{2}/\d{2}/\d{4}\b|\b\d{2}/\d{2}/\d{2}\b|\b\d{2}-\d{2}-\d{4}\b"
...
def converter_data_textual(data_texto):
    ...
    # Padrão mais flexível: DD [de] MES [de] AAAA ou DD/MES/AAAA, com ou sem "de" e aceita "/" ou espaços
    match = re.search(r"\b(\d{1,2})[/\s]*(?:de)?\s*([A-Z\u00C0-\u00FC]+)\s*(?:de)?[/\s]*(\d{4})\b", data_texto, re.IGNORECASE)
    if match:
...
def extrair_nome_do_texto(linhas):
    ...
    # 2) Origem tem segunda prioridade: tenta mesma linha primeiro
    ...
    # 3) Se 'Origem' aparece em linha isolada, pega próxima linha não vazia como nome
...
def extrair_valor_correto(texto):
    ...
    # 1) Tenta primeiro padrão "Valor:" seguido de número (com ou sem milhar)
    match = re.search(r"(?:Valor|VALOR)\s*[:\-]?\s*(R\$\s*)?(\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{2})?)", txt, re.IGNORECASE)
    if match:
    ...
    # 2) Padrão com R$ seguido de número (suporta milhar, centavos, ou valor simples como "R$ 5")
...
def validar_data_no_texto(texto):
    ...
    # Converte para maiúsculas para buscar meses
    texto_upper = texto_limpo.upper()

    # procura padrões reconhecidos
    m = re.search(PADRAO_DATA, texto_upper, re.IGNORECASE)
    if m:
...
def processar_imagem(caminho_imagem, pessoas):
    ...
    try:
        ...
        if caminho_imagem.lower().endswith('.pdf'):
            ...
            # Processa PDF usando pdfplumber
            texto = ""
            ...
            # Para imagens, usa PIL e pytesseract
            img = Image.open(caminho_imagem)
            # Tenta OCR com config padrão
            texto = pytesseract.image_to_string(img, lang="por", config="--psm 6")
            
            # Se não encontrou valor, tenta com PSM 3 (busca colunas de texto) que funciona melhor para valores grandes
            if not re.search(r"R\$\s*\d+", texto, re.IGNORECASE):
                texto_psm3 = pytesseract.image_to_string(img, lang="por", config="--psm 3")
                if re.search(r"R\$\s*\d+", texto_psm3, re.IGNORECASE):
                    # Se PSM 3 encontrou melhor, usa combinação dos dois
                    texto = texto + " " + texto_psm3
        ...
        # Valida que é um comprovante do Mercado Pago
        if extrair_instituicao_do_texto:
            inst_info = extrair_instituicao_do_texto(linhas)
            if inst_info.get("instituicao") and inst_info["instituicao"] != "MERCADOPAGO":
                resultado["erro"] = f"Comprovante não é do Mercado Pago, é de {inst_info.get('instituicao')}"
                return resultado
...
def main():
    ...
    for f in arquivos:
        ...
        # Se vem com caminho (tipo "inst/arquivo.pdf"), junta normalmente com DATA_DIR
        if os.sep in f:
            caminho = os.path.join(DATA_DIR, f)
        # Senão, assume que é arquivo simples na pasta DATA_DIR
        else:
            caminho = os.path.join(DATA_DIR, f)
